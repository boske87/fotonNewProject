...
@section('index_header')
    <h1>Galerija zvanja  {{$user->ime_prezime}}</h1>
            ...
            <table id="sortable-table" class="table">
                ...
                @foreach($items as $item)
                    <tr id="{{ $item->id }}" class="{{ $item->deleted_at ? 'danger' : ''}}">
                        <td>
                            <a class="fancybox" rel="group" href="{{ Image::load('gallery/galerija_zvanja'.$item->galleryId.'/' . $item->main_image, ['h' => 10]) }}"><img src="{{ Image::load('gallery/galerija_zvanja'.$item->galleryId.'/' . $item->main_image, ['h' => 10]) }}" width="150px" alt=""></a>
                        </td>
