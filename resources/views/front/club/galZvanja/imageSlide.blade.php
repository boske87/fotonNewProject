    ...
    <script>Galleria.loadTheme("{{ asset('assets/js/galleria.classic.min.js')}}");
        Galleria.ready(function(options) {
            ...
            this.bind('image', function(e) {
                ...
                $.getJSON('/foton-klub/getComents-call/'+e.galleriaData.original.id+'/'+$('#galleryId').val(), function(data) {
                    $('#ul_comments').html('');
                ...
                $.ajax({
                    type: 'POST',
                    url: '/foton-klub/updateImageAjax-call/view/'+e.galleriaData.original.id,
                    data:{
        ...
        $(document).on("keypress", "#unos_komentara", function (e) {
            if (e.keyCode == 13) {
                if ($("textarea[name='komentar']").val() != '') {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '/foton-klub/updateImageAjax-call/com/' + $('#imageId').val(),
                        data: {
                            '_token': $('meta[name="csrf-token"]').attr('content'),
                            'comment': comm,
